...
class OpenHighOpenLow:
    ...
    def sendgetopenhighopenlowmaail(self):
        try:
            ...
            <table style="width:100%"> <tr> <th>SYMBOL</th> <th> LAST TRADED PRICE </th> <th>OPEN</th> <th>HIGH</th> <th>LOW</th> <th>DAILYVOLATILITY</th> <th> VOLATILITYVALUE </th> <th>BUY/SELL</th>"""
            ...
            for value in nify50_json_obj:
                if value['open'] == value['high'] or value['open'] == value['low']:
                    ...
                    volatilitycal = (float(stockderivativedetail['dailyVolatility']) / 100) * float(str(value['open']).replace(',',''))
                    ...
                    <td>{value['high']}</td>
                    <td>{value['low']}</td>
                    <td>{stockderivativedetail['dailyVolatility']}</td>
